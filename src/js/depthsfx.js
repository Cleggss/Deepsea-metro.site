document.querySelectorAll("a:not(.navareastation a):not(.flex4button):not(footer a), .sitemaplinkdropdown, summary").forEach((el) => { //each element with classes specified
...
document.querySelectorAll(".navareastation a").forEach((el) => { //navbar area
...
document.querySelectorAll(".flex4button, footer a").forEach((el) => { //each .flex4button + footer link
    el.addEventListener("mouseenter", () => { //"mouseenter"
